...
export const useJobStore = create<IJobStore>((set, get) => ({
	...
	getJobs: async (
		...
		try {
			...
			if (industry && industry !== "all") {
				...
				jobs = jobs.filter((j) => {
					const jobIndustry = j.industry
						? String(j.industry).toLowerCase().trim()
						: "undefined";
	...
	getJobResumes: async (
		...
		try {
			...
			const jobResumes = (response.data?.data ??
				response.data ??
				[]) as IResume[];
			const jobResumesMeta = (response.data?.meta ??
				null) as IPaginationMeta | null;
	...
	generateJobContent: async (payload, accountId) => {
		...
		if (target === "description") {
			...
			set({
				loadingGenerateDescription: true,
				loadingGenerateRequirements: true,
			});
		}
		try {
			const response = await BackendApiClient.post(
	...
	getJobApplications: async (id, accountId) => {
		...
		try {
			...
			errorToast(
				resolveErrorMessage(error, "Failed to load job applications.")
			);
		} finally {
