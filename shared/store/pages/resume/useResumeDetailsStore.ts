...
const normalizeTopHighlight = (
	value: unknown
): ResumeAnalysisTopHighlight | string | null => {
...
const normalizeGaps = (value: unknown): ResumeAnalysisGap[] => {
	...
	return asArray(parsed).reduce<ResumeAnalysisGap[]>((acc, item) => {
		...
		const title =
			...
			asString(record.why_gap) ??
			asString(record.job_requirement) ??
			"Gap";
		acc.push({
			title,
			description: asString(record.description),
...
const normalizeSkillItems = (value: unknown): ResumeAnalysisSkillItem[] =>
	asArray(safeParseJson(value)).reduce<ResumeAnalysisSkillItem[]>(
		(acc, item) => {
			const record = asObject(item);
			if (!record) return acc;
...
const normalizeSkillDepth = (value: unknown): ResumeAnalysisSkillDepth[] =>
	asArray(safeParseJson(value)).reduce<ResumeAnalysisSkillDepth[]>(
		(acc, item) => {
			const record = asObject(item);
			if (!record) return acc;
			...
			acc.push({
				skill,
				depth: (asString(record.depth) ??
					"listed_only") as ResumeAnalysisSkillDepth["depth"],
...
const normalizeExperienceItems = (
	value: unknown
): ResumeAnalysisExperienceItem[] =>
	asArray(safeParseJson(value)).reduce<ResumeAnalysisExperienceItem[]>(
		(acc, item) => {
			const record = asObject(item);
			if (!record) return acc;
			acc.push({
...
const normalizeResumeResponse = (raw: unknown): ResumeDetails | null => {
	...
	const domainMatchRaw = asObject(
		safeParseJson(analysisRaw?.domain_match ?? analysisRaw?.domainMatch)
	);
	const experienceValidationRaw = asObject(
		safeParseJson(
			analysisRaw?.experience_validation ?? analysisRaw?.experienceValidation
		)
	);
	...
	const topStrengthNorm = normalizeTopHighlight(
		reviewRaw?.top_strength ??
			analysisRaw?.top_strength ??
			analysisRaw?.topStrength
	);
	const topConcernNorm = normalizeTopHighlight(
		reviewRaw?.top_concern ??
			analysisRaw?.top_concern ??
			analysisRaw?.topConcern
	);

	const strengthsNorm = normalizeStrengths(
		Array.isArray(analysisRaw?.strengths)
			? analysisRaw?.strengths
			: Array.isArray(insightsRaw?.strengths)
				? insightsRaw?.strengths
				: []
	);

	const gapsNorm = normalizeGaps(
		Array.isArray(analysisRaw?.gaps)
			? analysisRaw?.gaps
			: Array.isArray(insightsRaw?.gaps)
				? insightsRaw?.gaps
				: []
	);

	const redFlagsNorm = normalizeRedFlags(
		analysisRaw?.red_flags ?? analysisRaw?.redFlags
	);
	const skillDepthNorm = normalizeSkillDepth(
		analysisRaw?.skill_depth ?? analysisRaw?.skillDepth
	);

	const directMatchItems = normalizeSkillItems(
		groupedSkillsMatchedRaw?.direct_match
	);
	const semanticMatchItems = normalizeSkillItems(
		groupedSkillsMatchedRaw?.semantic_match
	);
	const missingSkillItems = normalizeSkillItems(groupedSkillsRaw?.missing);

	const matchedTechnicalSkills = Array.from(
		new Set([
			...asStringArray(analysisRaw?.matched_technical_skills),
			...asStringArray(
				analysisRaw?.matched_skills ?? analysisRaw?.matchedSkills
			),
	...
	const missingTechnicalSkills = Array.from(
		new Set([
			...asStringArray(analysisRaw?.missing_technical_skills),
			...asStringArray(
				analysisRaw?.missing_skills ?? analysisRaw?.missingSkills
			),
	...
	const normalizedAnalysis = analysisRaw
		? ({
				...
				job_type: asString(
					analysisRaw.job_type
				) as ResumeAnalysisModel["job_type"],
				score_breakdown:
					scoreBreakdown as ResumeAnalysisModel["score_breakdown"],
				dealbreakers: asStringArray(
					reviewRaw?.dealbreakers ?? analysisRaw.dealbreakers
				),
				seniority_level:
					asString(analysisRaw.seniority_level ?? analysisRaw.seniorityLevel) ??
					null,
				seniority_fit: asString(
					analysisRaw.seniority_fit ?? analysisRaw.seniorityFit
				) as ResumeAnalysisModel["seniority_fit"],
				recommendation: asString(analysisRaw.recommendation) ?? "consider",
				confidence: asString(
					analysisRaw.confidence
				) as ResumeAnalysisModel["confidence"],
				risk_level: asString(
					analysisRaw.risk_level ?? analysisRaw.riskLevel
				) as ResumeAnalysisModel["risk_level"],
				summary: asString(analysisRaw.summary) ?? null,
				profile_tagline:
					asString(analysisRaw.profile_tagline ?? analysisRaw.profileTagline) ??
					null,
				fit_label:
					asString(analysisRaw.fit_label ?? analysisRaw.fitLabel) ?? null,
				recommendation_label:
					asString(
						analysisRaw.recommendation_label ?? analysisRaw.recommendationLabel
					) ?? null,
				confidence_label:
					asString(
						analysisRaw.confidence_label ?? analysisRaw.confidenceLabel
					) ?? null,
				risk_label:
					asString(analysisRaw.risk_label ?? analysisRaw.riskLabel) ?? null,
				top_strength: topStrengthNorm,
				top_concern: topConcernNorm,
				review: {
					description:
						asString(reviewRaw?.description ?? analysisRaw.summary) ?? null,
					top_strength: topStrengthNorm,
					top_concern: topConcernNorm,
					dealbreakers: asStringArray(
						reviewRaw?.dealbreakers ?? analysisRaw.dealbreakers
				...
				experience: groupedExperienceRaw
					? {
							total_experience: normalizeExperienceItems(
								groupedExperienceRaw.total_experience
							),
							domain_experience: normalizeExperienceItems(
								groupedExperienceRaw.domain_experience
							),
							job_time_frame: normalizeExperienceItems(
								groupedExperienceRaw.job_time_frame
							),
						}
					: null,
				quantified_achievements: asArray(
					safeParseJson(analysisRaw.quantified_achievements)
				).reduce<ResumeAnalysisAchievement[]>((acc, item) => {
					const record = asObject(item);
					if (!record) return acc;
					acc.push({
						...
						category: asString(
							record.category
						) as ResumeAnalysisAchievement["category"],
					});
					return acc;
				}, []),
				analysis_summary_paragraph:
					asString(analysisRaw.analysis_summary_paragraph) ?? null,
				...
				strengths_count:
					asNumber(analysisRaw.strengths_count ?? analysisRaw.strengthsCount) ??
					strengthsNorm.length,
				gaps_count:
					asNumber(analysisRaw.gaps_count ?? analysisRaw.gapsCount) ??
					gapsNorm.length,
				matched_count:
					asNumber(analysisRaw.matched_count ?? analysisRaw.matchedCount) ??
					matchedTechnicalSkills.length,
				missing_count:
					asNumber(analysisRaw.missing_count ?? analysisRaw.missingCount) ??
					missingTechnicalSkills.length,
				domain_match:
					(domainMatchRaw as ResumeAnalysisDomainMatch | null) ?? null,
				experience_validation:
					(experienceValidationRaw as ResumeAnalysisExperienceValidation | null) ??
					null,
	...
	return {
		...
		job: asObject(root.job)
			? { title: asString(asObject(root.job)?.title) }
			: null,
		resume_structured: asObject(root.resume_structured)
			? {
					data:
						(asObject(
							asObject(root.resume_structured)?.data
						) as ResumeStructured | null) ?? null,
					email: asString(asObject(root.resume_structured)?.email) ?? null,
					phone: asString(asObject(root.resume_structured)?.phone) ?? null,
					current_title:
						asString(asObject(root.resume_structured)?.current_title) ?? null,
					city: asString(asObject(root.resume_structured)?.city) ?? null,
					country: asString(asObject(root.resume_structured)?.country) ?? null,
					total_experience_years:
						asNumber(
							asObject(root.resume_structured)?.total_experience_years
						) ?? null,
					languages: asStringArray(asObject(root.resume_structured)?.languages),
					language_levels: asStringArray(
						asObject(root.resume_structured)?.language_levels
					),
					top_degree:
						asString(asObject(root.resume_structured)?.top_degree) ?? null,
					top_institution:
						asString(asObject(root.resume_structured)?.top_institution) ?? null,
					education_country:
						asString(asObject(root.resume_structured)?.education_country) ??
						null,
					education_gpa:
						asString(asObject(root.resume_structured)?.education_gpa) ?? null,
				}
			: null,
	...
	getResume: async (id: number | string) => {
		...
		try {
			const response = await apiClient.get(
				`/api/candidate/resume/${requestId}`
			);
			set({
				resume: normalizeResumeResponse(response.data?.data),
				loading: false,
			});
		} catch {
			set({
